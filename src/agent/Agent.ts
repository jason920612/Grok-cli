...
export class Agent {
  ...
  async run(task: string, oneShot: boolean, signal?: AbortSignal, interjections?: Interjections, events?: AgentEventSink): Promise<string> {
    const loop = new AgentLoop(this.provider, this.config, this.context, this.tools, this.toolContext(), this.skillLoader, this.toolSkills, this.skillLoader.projectInstructions(), events, this.usage, undefined, interjections);
